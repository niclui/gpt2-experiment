...
class ModelArguments:
    ...
    custom_model: bool = field(
        default=False,
        metadata={"help": "Whether to use custom model args"},
    )
...
class DataTrainingArguments:
    ...
    tokenized_data_dir: Optional[str] = field(default=None, metadata={"help": "Data directory of tokenized data"})
    ...
    def __post_init__(self):
        if (self.dataset_name is None and self.train_file is None and self.validation_file is None and self.tokenized_data_dir is None) :
            raise ValueError("Need either a dataset name or a training/validation file, or a tokenized data dir.")
        else:
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        # This allows us to pass in a json of arguments which is much neater
    ...
    # treat train file as a the save directory
    logger.info("Loading dataset from disk")
    lm_datasets = load_from_disk(data_args.tokenized_data_dir)
    ...
    if training_args.do_train:
        ...
        # Can subset training data to learn with
        max_train_samples = (
